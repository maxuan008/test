...
!(function (name, definition) {
  ...
  * Examples:
  ...
  EventProxy.prototype.trigger = function (eventname, data) {
    ...
    console.log('事件触发了,触发函数为: trigger, 事件名：', eventname );
    console.log('开始while循环:');
    while (both--) {
      ev = both ? eventname : ALL_EVENT;
      console.log("1. ev, both", ev, both);
      list = calls[ev];
      ...
      if (list) {
        for (i = 0, l = list.length; i < l; i++) {
          ...
          if (!(callback = list[i])) {
            ...
            var args = [];
            var start = both ? 1 : 0;
            console.log("else执行: 1. ", start)
            console.log("else执行: 2. arguments: ", arguments)
            for (var j = start; j < arguments.length; j++) {
              args.push(arguments[j]);
            }
            console.log("else执行: 2. args: ",args);
